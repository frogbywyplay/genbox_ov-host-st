...
glibc_do_configure() {
	...
	[[ $(tc-arch) != "sh" ]] && myconf="${myconf} --cache-file=${WORKDIR}/config.wybox-x86"
...
src_unpack() {
	...
	# ST patches
	if [[ $(tc-arch) == "sh" ]]; then
		epatch ${FILESDIR}/st_patches/glibc-2.5-ustat.patch
		epatch ${FILESDIR}/st_patches/glibc-2.5-memcpy.patch
		epatch ${FILESDIR}/st_patches/glibc-2.5-shsyscall2.patch
		#epatch ${FILESDIR}/st_patches/glibc-2.3.6-nptl-config.patch
		epatch ${FILESDIR}/st_patches/glibc-2.5-sh-atomic-fixes.patch
		epatch ${FILESDIR}/st_patches/glibc-2.5-mutex.patch
		epatch ${FILESDIR}/st_patches/glibc-2.5-clone.patch
		epatch ${FILESDIR}/st_patches/glibc-2.5-fegetexceptflag.patch
	else
